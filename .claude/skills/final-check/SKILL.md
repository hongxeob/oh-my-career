...
6. `[고유]` **불필요·중복·과잉 상세·리스크**: 아래 네 가지를 **각각 따로** 훑는다. "중복 없음" 같은 한 줄 결론을 내지 말고, 항목마다 원문을 인용하거나 "해당 없음"을 명시한다.
   ...
   **(a) 중복** — 특히 **제목과 자식이 같은 말을 하는지**. refine이 bullet을 통합하면서 제목에 올린 문구가
   자식에 그대로 남는 일이 흔하다. 제목의 명사구를 자식에서 `grep`해 겹치는 것을 찾는다.
   같은 수치가 두 bullet에 반복되는 경우, 뒤쪽은 라벨을 줄일 수 있는지 본다.
   고유명사(도구명, 제품명)가 문서 전체에서 3회 이상 나오면 한 곳은 줄인다.

   **(b) 과잉 상세 — "면접에서 말하면 되는 것"** — 이력서는 면접을 부르는 문서이지 면접 답변지가 아니다.
   아래는 빼고 면접 대화의 여지로 남긴다. 빼도 성과 주장은 그대로 서야 한다.
   - 명령어·API·함수 이름 수준의 구현 디테일
   - 진단이 뒤집힌 경위를 3줄 이상 서술한 것 (결론과 근거 1줄이면 된다)
   - 레거시 코드의 라인 수, 절 개수 같은 고고학적 수치
   - 절차의 절차 (정정 절차 안에 포함된 세부 단계 등)
   ⚠️ **수치와 성과 주장은 빼지 않는다.** 빼는 것은 설명·수식·경위다. 작업 후 핵심 수치를 전수 `grep`해
   하나도 빠지지 않았음을 확인하고 그 목록을 리포트에 남긴다.

   **(c) 구조 균형** — 부모 bullet별 자식 수를 센다. 한 bullet만 튀면(다른 것이 2-4개인데 혼자 8개)
   쪼개거나 줄인다. refine이 블록을 편입하면서 한쪽이 비대해지는 일이 잦다.

   **(d) 리스크** — 직무 무관 정보, 과장·애매한 책임 범위 표현.
