...
# 진행하기로 했으면 이것만 치면 된다. PDF까지 알아서 이어진다
/draft-resume
...
**`/draft-resume` 하나면 PDF까지 갑니다.** 파이프라인이 스스로 이어지고, 사람이 봐야 하는 곳에서만 멈춥니다.
...
/draft-resume → /verify-resume → /cross-verify
                                      ↓
                          🛑 BLOCK이면 여기서 멈춘다 (오귀속은 제출 사고로 직결)
                          PASS면 계속 ↓
                    /review-resume → /refine-resume → /final-check
                                                           ↓
                                          🛑 '낮음'이면 여기서 멈춘다
                                          '높음/보통'이면 계속 ↓
                                                     /pdf-resume  ✅
...
각 단계는 결과물을 `outcome/{company}/` 하위 폴더에 저장하고, 끝나면 **어느 단계에서 무엇을 바꿨는지 한 번에
보고합니다.** 중간에 끼어들어 지시하면 그쪽이 우선합니다. 개별 스킬을 따로 돌리고 싶으면 그 커맨드만 치면 됩니다.
