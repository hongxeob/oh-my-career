...
| 산출물 | 목표 | 상한(하드) |
|--------|------|-----------|
| 이력서 제출본 (`4_refine`, `5_pdf`) | **2페이지** | **3페이지** |
| 포트폴리오 (`6_portfolio`) | 3-6페이지 | 6페이지 |

**3페이지는 상한이지 목표가 아니다.** 2페이지에 들어가면 2페이지로 낸다. 3페이지를 허용하는 이유는
분량을 맞추려다 **JD 접점이 있는 내용을 버리는 것이 더 큰 손해**이기 때문이다. 실제로 그런 적이 있다 —
2페이지를 맞추느라 차별화 포인트 1순위(측정 방법론, 자기 수치 철회)를 통째로 잘라냈다.

렌더 전 예산은 **본문 HTML 약 6KB당 1페이지**다 (2026-09 실측: 12,114 B → 정확히 2p).
페이지 수를 실제로 재는 곳은 `_shared/render-pdf.sh` **하나뿐**이고, 나머지 단계의 판단은 전부 추정이다.

⚠️ **이 숫자를 스킬 파일에 복사하지 않는다.** 예전에 `2페이지`가 5개 파일에 흩어져 있었다.
스킬은 "CLAUDE.md 「분량 기준」"이라고만 쓴다. 유일한 예외는 `pdf-resume`가 게이트에 넘기는 인자인데,
그건 **강제하는 쪽**이라 값이 있어야 하고, 그 자리에 출처를 주석으로 달아둔다.
